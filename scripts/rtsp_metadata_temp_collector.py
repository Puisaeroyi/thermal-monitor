...
def process_camera(
    ...
    for row in deduped_rows.values():
        ...
        payload = {
            ...
            "unit": "Fahrenheit",
            "min_temperature_celsius": min_celsius,
            "max_temperature_celsius": max_celsius,
            "avg_temperature_celsius": avg_celsius,
            "method": "rtsp_metadata_snapshot",
        }
        try:
            coordinates = resolve_row_coordinates(camera, str(row["roi"]))
        except Exception as exc:
            log(f"[{camera.camera_name}] Failed to resolve ROI coordinates: {exc}")
            coordinates = None

        if coordinates is not None:
            payload["roi_index"] = coordinates["roi_index"]
            payload["coordinates"] = coordinates["coordinates"]
        ...
        with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    if emitted == 0:
                        log(f"[{camera.camera_name}] No metadata temperature rows emitted in this cycle")
                except Exception as exc:
